        ...
        match &self {
            ...
            BasicStmt::AssignStmt(left_val, init_exp) => {
                ...
                let left_val_ptr = match result1 {
                    IRLeftValBuildResult::Const(_) | IRLValBuildResult::TempVal(_) => {
                        return Err(format!(
                ...
                //处理初始化值，即右值
                let result2 = init_exp.build(program, ctx)?;
            ...
            BasicStmt::IfStmt(bool_expr, stmt1, possible_stmt2) => {
                ...
                let block_end = create_block(program, ctx, "if_block_end");
                ...
                let block1 = create_new_block(program, ctx, "if_block_1");
                ...
                let jmp_block = match &**possible_stmt2 {
                    Some(stmt2) => {
                        let block2 = create_block(program, ctx, "if_block_2");
                        create_basic_block(program, ctx, [block2]);
                ...
                let if_stmt = create_new_local_value(program, ctx).branch(bool_expr_value, block1, jmp_block);
                ...
                create_basic_block(program, ctx, [block_end]);
            ...
            BasicStmt::ReturnStmt(returned_exp) => {
                ...
                emit_instructions(program, my_ir_generator_info, [return_stmt]);
                Ok(IRBuildResult::EARLYSTOPPING)
            }
        }
    }
